...
#show: ilm.with(
  ...
  bibliography: bibliography("references.yaml"),
...
Mô hình `ParaConv` là một kiến trúc All-convolutional (thuần tích chập) được thiết kế để duy trì luồng gradient liên tục, tương thích tốt với các thuật toán xấp xỉ đạo hàm như QuickProp. Thay vì sử dụng phép gộp (pooling) gây ra sự gián đoạn về mặt không gian toán học, mạng sử dụng các lớp tích chập có bước nhảy (stride $s=2$) để giảm độ phân giải @springenberg2014allconv. Phần đầu ra sử dụng lớp tích chập $1 times 1$ và Global Average Pooling (GAP) đóng vai trò regularizer cấu trúc, giảm overfitting so với fully-connected @lin2013nin để trực tiếp trích xuất logits $z$.

= Quá trình Lan truyền thuận (Forward Pass)
== Lớp tích chập 2D
Cho đầu vào $X$, trọng số kernel $bold(w)$ kích thước $k times k$ và bias $b$. Tại vị trí $(h, w)$ của kênh đầu ra $c_"out"$, với bước nhảy $s$ và phần đệm $p$, quá trình lan truyền thuận là phép tương quan chéo (cross-correlation):
$
  Y_(c_"out", h, w) = b_(c_"out") + sum_(c_"in") sum_(m=0)^(k-1) sum_(n=0)^(k-1) w_(c_"out", c_"in", m, n) X_(c_"in", h dot s + m - p, w dot s + n - p)
$
Kích thước không gian đầu ra:
$
  H_"out" = floor((H_"in" + 2p - k) / s) + 1, quad W_"out" = floor((W_"in" + 2p - k) / s) + 1
$

== Hàm kích hoạt SiLU
Gọi hàm sigmoid logistic là $g(u) = 1 / (1 + e^(-u))$, hàm SiLU được định nghĩa:
$ "SiLU"(u) = u dot g(u) $
Đây là hàm trơn, liên tục và đạo hàm không bị triệt tiêu đột ngột, đảm bảo tính ổn định khi tính toán sai phân đạo hàm trong thuật toán QuickProp. @ramachandran2017swish

== Tích chập 1x1 và GAP
Lớp tích chập $1 times 1$ biến đổi thành $K$ kênh. GAP tính trung bình không gian @lin2013nin:
...
Ký hiệu $Delta^{(t)} = (partial cal(L)) / (partial bold(w))$ là đạo hàm riêng phần tại bước lặp $t$. *(Để đơn giản, các công thức dưới đây trình bày cho một mẫu dữ liệu; với dạng mini-batch, ta chỉ cần lấy tổng theo chỉ số của các mẫu trong batch).*
...
2. Gradient đối với trọng số (Weights):
  $
    (partial cal(L)) / (partial w_(c_"out", c_"in", m, n)) = sum_(h, w) delta^"out"_(c_"out", h, w) X_(c_"in", h dot s + m - p, w dot s + n - p)
  $
3. Gradient đối với đầu vào (Input):
  ...
  $
    (partial cal(L)) / (partial X_(c_"in", h, w)) = sum_(c_"out") sum_(m=0)^(k-1) sum_(n=0)^(k-1) delta^"out"_"dilated"_(c_"out", h + m - p, w + n - p) w_(c_"out", c_"in", k-1-m, k-1-n)
  $
...
QuickProp sử dụng kỹ thuật xấp xỉ đạo hàm riêng phần bằng một parabol @fahlman1988quickprop. Gọi $Delta^{(t)}$ và $Delta^{(t-1)}$ lần lượt là đạo hàm tại bước hiện tại và bước trước; $C^{(t-1)}$ là biến thiên trọng số bước trước. Mục tiêu là nhảy thẳng đến điểm cực tiểu của parabol, nơi đạo hàm $Delta^{(t+1)} = 0$.
...
- $mu$: Hệ số kẹp bước nhảy tối đa (thường chọn $mu = 1.75$ @fahlman1988quickprop) để tránh bùng nổ khi độ cong parabol quá nhỏ.
...
== Mô hình và khởi tạo
Mô hình `ParaConv` ($c_"base" = 16$) dùng kiến trúc thuần tích chập (All-convolutional) @springenberg2014allconv kết hợp hàm kích hoạt SiLU. Hàm SiLU được chọn vì nó trơn và dễ tính đạo hàm, giúp quá trình truyền ngược ổn định hơn.

Trọng số của mạng được khởi tạo theo phương pháp Xavier (Glorot) @glorot2010xavier:
$ bold(w) tilde cal(N)(0, sigma^2), quad sigma^2 = 2 / (n_"in" + n_"out") $
...
1. Full-batch (Cập nhật 1 lần/epoch): Đây là cách làm gốc của QuickProp @fahlman1988quickprop. Đạo hàm được cộng dồn trên toàn bộ tập dữ liệu rồi mới cập nhật trọng số, giúp triệt tiêu hoàn toàn nhiễu:
  $
